...
async function main() {
  ...
  await test("All five exact statements and authors render without notes or analytics",async()=>{
    ...
    assert.deepEqual(nodes(view.tree).filter(n=>n.props.className==="mindset-author").map(text),[
      "\u2014 Roland-Garros","\u2014 Billie Jean King","\u2014 Serena Williams","\u2014 Kobe Bryant","\u2014 Michael Schumacher"
    ]);
    assert.equal(radios(view).length,5);assert.deepEqual(selected(view),[]);
  ...
  await test("Backup and restore automatically include the new key losslessly",async()=>{
    const e=environment({[KEY]:'{"2026-09-03":"pressure","2026-09-02":"rest"}',"jnl:v2:trades":"[]"});
